-- PATCH 9.2 — Q5 Time to signal buckets
...
buckets as (
  select
    coalesce(metadata->>'time_bucket', 'unknown') as time_bucket,
    count(*)::numeric as event_count,
    count(distinct metadata->>'decision_request_id')::numeric as unique_decision_count
  from signal_events
  group by 1
)
select r.dia_referencia, b.time_bucket, b.event_count, b.unique_decision_count
from buckets b cross join reference_day r
