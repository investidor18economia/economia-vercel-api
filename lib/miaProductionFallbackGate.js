...
const FAMILY_MODE_INTENT = Object.freeze({
  ...
  social_conversation: { mode: "social_conversation", intent: "social_conversation" },
  emotional_support: { mode: "emotional_support", intent: "emotional_support" },
  clarification: { mode: "clarification", intent: "clarification" },
  mixed_intent: { mode: "mixed_intent", intent: "mixed_intent" },
});
...
const CONVERSATIONAL_ROUTING_HINTS = new Set([
  ...
  "social_conversation_anchored",
  "emotional_support_reply",
  "emotional_support_anchored",
  "clarification_reply",
  "clarification_anchored",
  "cognitive_explanation_anchored",
...
function detectFamilyFromIntentRecognition(intentRecognition = null) {
  if (!intentRecognition || !shouldBypassDefaultProductSearch(intentRecognition)) {
    return null;
  }

  const act = intentRecognition.primaryIntent || intentRecognition.interactionMode;
  if (!act || !isNonCommercialInteractionMode(intentRecognition.interactionMode)) {
    return null;
  }

  return {
    family: act,
    source: `intent_recognition:${intentRecognition.interactionMode}`,
  };
}
...
export function detectRecognizedConversationalFamily(input = {}) {
  ...
  const candidates = [
    detectFamilyFromIntentRecognition(intentRecognition),
