/**
 * PATCH 11A — Intent Recognition Layer
 *
...
function detectActiveCommercialAsk(message = "") {
  const q = normalizeMessage(message);
  return /\b(preciso|precisamos|quero|queria|procuro|procurar|buscar|busco|escolher|comparar|compare|compara|versus|\bvs\b|vale\s+(?:\w+\s+)*a\s+pena|recomend\w*|indique|indica|melhor preco|melhor preço|oferta|promo\w*|me ajuda|ajuda com|ajudar com|ajuda a escolher)\b/.test(
    q
  );
}
...
function detectCasualCommentarySignal(message = "") {
  const q = normalizeMessage(message);
  if (!q || hasCommercialTokens(q)) return false;
...
function resolveInteractionMode({
  ...
  return {
    ...
    ambiguity: 0.55,
    requiresClarification: false,
    reasons: ["no_dominant_social_signal_default_commerce_watch"],
  };
}
