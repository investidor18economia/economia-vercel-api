...
export function hasMinimalProductIdentity(product = {}, { rawQuery = "" } = {}) {
  ...
  const normalizedQuery = normalizeProductKey(rawQuery || "");
  if (normalizedQuery && normalizedName === normalizedQuery) return false;
...
export function validateSpecificProductLockCandidate({
  ...
  if (lock.matchSource === "query_identity_anchor") {
    ...
    const queryKey = normalizeProductKey(query);
    if (identityName && queryKey && identityName === queryKey) {
...
export function resolveSpecificProductLock(input = {}) {
  ...
  return finalizeSpecificProductLock(query, {
    active: true,
    version: SPECIFIC_PRODUCT_RESOLUTION_LOCK_VERSION,
    query,
...
export function bootstrapSpecificProductLock(input = {}) {
  ...
  if (baseLock.active) return finalizeSpecificProductLock(query, baseLock);
