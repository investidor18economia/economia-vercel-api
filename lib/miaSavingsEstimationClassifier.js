...
export function buildWinnerVsMinimumEstimation(offerSet = {}, priceIntel = {}, context = {}) {
  ...
  return {
    ...
    savings_valid: eligible && direction !== MIA_SAVINGS_COMPARISON_DIRECTION.UNKNOWN,
    ...
    estimation_valid: sampleCount > 0 || bool(offerSet.winner_present) === true,
  };
}

/**
 * Mirror miaEstimatedSavings UI rule server-side (observation only — UNVERIFIED).
 * Uses winner_price when present; otherwise minimum_price from offer_set sample (documented fallback).
 * @param {Record<string, unknown>} offerSet
 * @param {{ requestId?: string|null, decisionRequestId?: string|null }} [context]
 */
export function buildUiAssumptionEstimation(offerSet = {}, context = {}) {
  const winnerPrice = num(offerSet.winner_price);
  const minimumPrice = num(offerSet.minimum_price);
  const referencePrice = winnerPrice ?? minimumPrice;
  const referenceSource =
    winnerPrice != null ? "WINNER_PRICE" : minimumPrice != null ? "MINIMUM_OFFER_FALLBACK" : null;
  const currency = offerSet.price_currency === MIA_SAVINGS_OFFICIAL_CURRENCY
    ? MIA_SAVINGS_OFFICIAL_CURRENCY
    : null;
  ...
  if (eligible && referencePrice != null) {
    savingsPercent = roundPercent(pickSavingsPercent(referencePrice) * 100);
    savingsAmount = computeEstimatedSavingsAmount(referencePrice);
  }
  ...
  return {
    ...
    ui_reference_price_source: referenceSource,
    shipping_included: false,
...
export function buildSavingsEstimationsFromOfferSetMetadata(
  ...
  const sampleCount = num(offerSetMetadata.price_sample_count) ?? 0;
  const winnerPresent = bool(offerSetMetadata.winner_present) === true;
  const estimations = [];

  if (sampleCount > 0 || winnerPresent) {
    estimations.push(
      buildWinnerVsMinimumEstimation(offerSetMetadata, priceIntelMetadata, context)
    );
  }

  const uiEstimation = buildUiAssumptionEstimation(offerSetMetadata, context);
  if (uiEstimation.estimation_valid) {
    estimations.push(uiEstimation);
  }
