 ...
 * PATCH 3.4 — user_authenticated milestone for retention foundation.
 */
...
import {
  ...
  buildMiaUserAuthenticatedPayload,
  isAnalyticsUuid,
...
/**
 * PATCH 3.4 — Records verified login milestone (retention foundation).
 * Fires on each successful OTP login; user_id resolved server-side.
 *
 * @param {{ authUser?: { session_token?: string|null, id?: string|null }|null }} [options]
 */
export async function trackMiaUserAuthenticated(options = {}) {
  await trackMiaEvent("user_authenticated", buildMiaUserAuthenticatedPayload(), {
    conversationId: false,
    authUser: options.authUser || null,
  });
}
