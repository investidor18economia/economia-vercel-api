 ...
 * PATCH 3.2 — conversation_id scoped to MIA chat thread (localStorage, shared with API).
 *
...
/** Shared with MIA chat API — single source of truth for the active conversation thread. */
...
function createAnalyticsConversationId() {
  if (typeof crypto !== "undefined" && typeof crypto.randomUUID === "function") {
    return crypto.randomUUID();
  }
...
function readConversationIdFromStorage(storage) {
  if (!storage || typeof storage.getItem !== "function") return null;

  try {
    const existing = storage.getItem(MIA_CONVERSATION_ID_KEY);
    if (isAnalyticsUuid(existing)) {
      return existing.trim();
    }
  } catch {
...
function persistConversationId(storage, conversationId) {
  if (!storage || typeof storage.setItem !== "function" || !conversationId) return;

  try {
    storage.setItem(MIA_CONVERSATION_ID_KEY, conversationId);
  } catch {
    // Analytics must never break the app if storage is unavailable.
  }
}
...
/**
 * Returns the current conversation UUID when valid. Does not create a new conversation.
 */
export function getCurrentAnalyticsConversationId() {
  if (typeof window === "undefined") return null;

  try {
    return readConversationIdFromStorage(window.localStorage);
  } catch {
    return null;
  }
}
...
/**
 * Returns a conversation UUID, creating one when missing or when legacy non-UUID values exist.
 * Official birth point for a new analytics conversation thread.
 */
export function getOrCreateAnalyticsConversationId() {
  if (typeof window === "undefined") return null;
  ...
  try {
    ...
    return createAnalyticsConversationId();
  }

  const existing = readConversationIdFromStorage(storage);
  if (existing) return existing;

  const conversationId = createAnalyticsConversationId();
  if (!conversationId) return null;

  persistConversationId(storage, conversationId);
  return conversationId;
}

/**
 * Starts a new conversation thread with a fresh UUID. Used by explicit "new conversation" flows.
 */
export function startNewAnalyticsConversation() {
  if (typeof window === "undefined") return null;
  ...
  try {
    persistConversationId(window.localStorage, conversationId);
  } catch {
    // fall through — return in-memory id
  }
 ...
 * @param {{ conversationId?: string|null|false, ensureConversation?: boolean }} [options]
 */
export async function trackMiaEvent(eventName, payload = {}, options = {}) {
  try {
    ...
    } else if (options.ensureConversation) {
      resolvedConversationId = getOrCreateAnalyticsConversationId();
    } else {
      const current = getCurrentAnalyticsConversationId();
      if (current) resolvedConversationId = current;
    }
