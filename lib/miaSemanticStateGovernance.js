...
const COMMERCIAL_CONTINUATION_PATTERN =
  /\b(vale mais(?:\s+a\s+pena)?|qual vale mais|qual [eé] melhor|s[oó] me diz|compara|compare|melhor op[cç][ãa]o|qual deles|qual desses|entre os dois|entre esses|mais barato|runner.?up|segundo lugar|e a c[aâ]mera|e o pre[cç]o)\b/i;
...
function inferCommercialAnchorValidity(sessionContext = {}, options = {}) {
  ...
  if (sessionContext.decisionCompleted && !options.allowCompletedDecisionReuse) {
...
export function normalizeSemanticSessionState(sessionContext = {}, options = {}) {
  ...
  const commercialAnchorCheck = inferCommercialAnchorValidity(sessionContext, options);
