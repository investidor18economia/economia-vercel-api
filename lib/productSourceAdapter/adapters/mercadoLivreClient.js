 ...
 *
 * PATCH Comercial 2B / 05J.6 — Mercado Livre real client (Vault runtime integration)
 *
 * Busca pública via /sites/{siteId}/search.
 * OAuth bearer tokens resolve from Provider Credential Vault when persistence is enabled.
 */
...
import { sanitizeProviderCredentialDiagnostics } from "../../server/providerCredentialSanitization.js";
import {
...
export const MERCADOLIVRE_CLIENT_RUNTIME_VERSION = "05J.9";
 ...
 */
function buildMercadoLivreCredentialDiagnostics(resolution = {}) {
  return sanitizeProviderCredentialDiagnostics({
    credentialSource: resolution.source || "missing",
    readiness: resolution.readiness || null,
    reasonCode: resolution.reasonCode || null,
    expiresAt: resolution.expiresAt ?? null,
    credentialVersion: resolution.credentialVersion ?? null,
    accessTokenPresent: !!resolution.accessToken,
    refreshDiagnostics: resolution.refreshDiagnostics || null,
  });
}

/**
 * Resolve runtime bearer credentials — vault-first when persistence is configured.
 * No module-level cache; decrypt-on-demand only.
 *
 * @param {Record<string, string|undefined>} [env]
 * @param {Record<string, unknown>} [options]
 */
export async function resolveMercadoLivreClientRuntimeCredentials(env = process.env, options = {}) {
  ...
  return {
    ...
    credentialSource: resolution.source || "missing",
    credentialReadiness: resolution.readiness || null,
    credentialReasonCode: resolution.reasonCode || null,
    credentialDiagnostics: buildMercadoLivreCredentialDiagnostics(resolution),
  };
}
...
export function validateMercadoLivreEnv(env = process.env) {
  ...
  return {
    ...
    hasAccessToken: isMercadoLivreOAuthTokenPersistenceConfigured(env),
  };
}
...
/**
 * @param {Record<string, string|undefined>} [env]
 * @param {ReturnType<typeof readEnv>|Awaited<ReturnType<typeof resolveMercadoLivreClientRuntimeCredentials>>} [runtimeConfig]
 */
export function buildMercadoLivreRequestHeaders(env = process.env, runtimeConfig = null) {
  const config = runtimeConfig || readEnv(env);
  const headers = {
...
async function executeMercadoLivreSearchRequest({
  ...
  const timeoutId = controller
    ...
    : null;
  const requestHeaders = buildMercadoLivreRequestHeaders(env, config);
...
export async function searchMercadoLivreProducts(query = "", limit = 12, options = {}) {
  ...
  const runtimeConfig = await resolveMercadoLivreClientRuntimeCredentials(env, options);
  const cap = clampLimit(limit);
  ...
  onStage?.("http_request_started", { requestUrl: sanitizeRequestUrl(url, runtimeConfig) });
  ...
  return {
    ...result,
    credentialDiagnostics: runtimeConfig.credentialDiagnostics,
    clientInvoked: true,
...
export async function getMercadoLivreProductById(productId = "", options = {}) {
  ...
  const response = await executeMercadoLivreJsonGet({ url, env, fetcher, config: runtimeConfig });
  if (!response.ok) {
    return {
      ...response,
      productId: id,
...
export function redactMercadoLivreSecrets(value = "", env = process.env) {
  const config = readEnv(env);
  let safe = sanitizeMercadoLivreSensitiveOutput(value, config);
  const legacyEnvToken = normalizeMercadoLivreAccessToken(env?.MERCADOLIVRE_ACCESS_TOKEN);
  if (legacyEnvToken) {
    safe = sanitizeForOutput(safe, legacyEnvToken);
  }
  return safe;
}
