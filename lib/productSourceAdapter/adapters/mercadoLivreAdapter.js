...
export async function fetchMercadoLivreCommercialAdapterResult({
  ...
  const runtimeEnv = env || getActiveRequestExecutionEnv();
  if (!isCommercialProviderEnabled(COMMERCIAL_PROVIDER_IDS.MERCADOLIVRE_PUBLIC, runtimeEnv)) {
    return {
      ...buildMercadoLivreProviderDisabledResult({
        provider: MERCADO_LIVRE_PROVIDER,
      }),
              ...
              if (!searchResult.ok) {
                return attachExecutionTelemetry(
                  buildMercadoLivreCommercialNeutralResult(searchResult.error || "provider_error", {
                    ...
                    searchMode: mode,
                    registryMetadata: getMercadoLivreCommercialRegistryMetadata(runtimeEnv),
                  }),
                  searchResult
                );
              }
