 ...
 * @param {Record<string, unknown>} [product]
 */
export function mapMercadoLivrePublicOfferToMergedOffer(product = {}) {
  return cloneOffer({
    source: product.source || COMMERCIAL_PROVIDER_IDS.MERCADOLIVRE_PUBLIC,
    title: cleanText(product.title || product.product_name || product.normalizedName || ""),
    price: product.price ?? product.numericPrice ?? null,
    image: product.image || product.thumbnail || null,
    url: cleanText(product.url || product.link || ""),
    brand: product.brand ?? null,
    seller: product.seller ?? null,
    category: product.category ?? null,
    provider: COMMERCIAL_PROVIDER_IDS.MERCADOLIVRE_PUBLIC,
    externalId: product.externalId ?? null,
  });
}
...
function resolveProviderGate(options = {}) {
  if (options.providerEnabled && typeof options.providerEnabled === "object") {
    return {
      ...
      mercadolivrePublic:
        options.providerEnabled[COMMERCIAL_PROVIDER_IDS.MERCADOLIVRE_PUBLIC] !== false,
      apifyMercadoLivre:
  ...
  return {
    ...
    mercadolivrePublic: isCommercialProviderEnabled(COMMERCIAL_PROVIDER_IDS.MERCADOLIVRE_PUBLIC),
...
export function validateCommercialMergeRegistry() {
  ...
  const mercadolivre = validateCommercialMergeProvider(COMMERCIAL_PROVIDER_IDS.MERCADOLIVRE_PUBLIC);
  ...
  return {
    ...
    mercadolivrePublic: mercadolivre,
 ...
 *   mercadolivrePublicOffers?: unknown[],
...
export function mergeCommercialOfferBundle(input = {}, options = {}) {
  ...
  const googleShoppingOffers = Array.isArray(input.googleShoppingOffers)
    ...
    : [];
  const mercadolivrePublicOffers = Array.isArray(input.mercadolivrePublicOffers)
    ? input.mercadolivrePublicOffers
    : [];
  ...
  const googleOffers = gate.googleShopping
    ...
    : [];
  const mercadolivreOffers = gate.mercadolivrePublic
    ? mapOfferList(mercadolivrePublicOffers, mapMercadoLivrePublicOfferToMergedOffer)
    : [];
  const apifyOffers = gate.apifyMercadoLivre
    ? mapOfferList(apifyMercadoLivreOffers, mapApifyMercadoLivreOfferToMergedOffer)
    : [];

  const offers = [...googleOffers, ...mercadolivreOffers, ...apifyOffers];
  const providersUsed = [];

  if (gate.googleShopping && googleOffers.length) {
    providersUsed.push(COMMERCIAL_PROVIDER_IDS.GOOGLE_SHOPPING);
  }
  if (gate.mercadolivrePublic && mercadolivreOffers.length) {
    providersUsed.push(COMMERCIAL_PROVIDER_IDS.MERCADOLIVRE_PUBLIC);
  }
