...
function hasAccessToken(env = process.env) {
  return !!String(env?.MERCADOLIVRE_ACCESS_TOKEN || "").trim();
}
...
export function validateMercadoLivreRequestHeaders(headers = {}, env = process.env) {
  const accessToken = String(env?.MERCADOLIVRE_ACCESS_TOKEN || "").trim();
  ...
  return {
    ...
    authHeaderSent: !!accessToken && !!bearerValue,
    invalidEmptyBearer,
    authorizationWouldBeSent: !!accessToken,
  };
}
...
export function inspectMercadoLivreForbiddenResponse(input = {}) {
  const safeErrorBodyPreview = sanitizeMercadoLivreErrorBody(
    ...
    input.config || {
      accessToken: String(input.env?.MERCADOLIVRE_ACCESS_TOKEN || "").trim(),
...
export function buildMercadoLivrePublicProbePlan(input = {}) {
  ...
  return {
    ...
    authMode: "public_api",
    accessTokenConfigured: hasAccessToken(env),
    blockedProviders: [
...
export function buildMercadoLivreAuthenticatedProbePlan(input = {}) {
  const env = input.env || process.env;
  const query = cleanText(input.query || "iPhone 13") || "iPhone 13";
  const tokenConfigured = hasAccessToken(env);

  return {
    version: MERCADOLIVRE_403_PROTECTED_FETCH_AUDIT_VERSION,
    mode: "authenticated_optional_token",
    probeEnabled:
      tokenConfigured &&
    ...
    requiredEnv: {
      ...
      MERCADOLIVRE_ACCESS_TOKEN: "(must be configured — never logged)",
      COMMERCIAL_RUNTIME_MODE: "controlled",
      COMMERCIAL_PROVIDER_MERCADOLIVRE_ENABLED: "true",
      COMMERCIAL_DEV_REAL_EXTERNAL_CALLS_ENABLED: "true",
      SERPAPI_KEY: "",
    ...
    cancelHint: "Requires MERCADOLIVRE_ACCESS_TOKEN and COMMERCIAL_ML_AUTHENTICATED_PROBE_ENABLED=true.",
  };
}
