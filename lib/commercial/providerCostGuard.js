...
export function buildProviderCostGuardDecision(input = {}) {
  ...
  if (
    ...
    !credentials.ready &&
    input.enforceCredentialReadiness === true
  ) {
    return finalizeDecision(base, {
      decision: PROVIDER_COST_GUARD_DECISIONS.BLOCK,
      reasonCode: PROVIDER_COST_GUARD_REASON_CODES.BLOCKED_BY_MISSING_CREDENTIALS,
  ...
  if (isMiaTestMode(env)) {
    const testPolicy = evaluateExternalProviderExecutionPolicy({
      providerId,
      env,
      explicitExternalAuthorization: input.explicitExternalAuthorization === true,
      explicitPaidAuthorization: hasExplicitPaidProviderOptIn,
      isObservabilityOnly,
    });
    if (!testPolicy.allowed) {
      return finalizeDecision(base, {
        ...
        shouldCallProvider: false,
        externalCallPrevented: true,
        externalExecutionPolicy: testPolicy,
        blockCategory: testPolicy.category,
      });
    }
  }
