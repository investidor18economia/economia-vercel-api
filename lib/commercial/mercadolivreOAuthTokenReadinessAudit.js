...
export const MERCADOLIVRE_OAUTH_TOKEN_READINESS_AUDIT_VERSION = "05J.9";
...
export function inspectMercadoLivreTokenPresence(env = process.env) {
  ...
  return {
    vaultConfigured,
    accessTokenPresent: vaultConfigured,
    accessTokenSource: vaultConfigured ? "vault" : null,
    accessTokenLength: 0,
    accessTokenFingerprint: null,
    refreshTokenPresent: vaultConfigured,
...
export function inspectMercadoLivreAuthHeaderReadiness(env = process.env) {
  ...
  return {
    authHeaderWillBeSent: vaultConfigured,
    authHeaderSentWhenBuilt: vaultConfigured,
    invalidEmptyBearer: false,
    userAgentSent: true,
    tokenShapeValid: vaultConfigured,
    duplicateBearerRisk: false,
  };
}
...
export function classifyMercadoLivreOAuthReadiness(input = {}) {
  ...
  if (!tokenPresence.vaultConfigured) {
    blockers.push("vault_unavailable");
  ...
  if (blockers.includes("vault_unavailable")) {
  ...
  } else if (tokenPresence.vaultConfigured) {
  ...
  return {
    ...
    vaultConfigured: tokenPresence.vaultConfigured,
    accessTokenPresent: tokenPresence.accessTokenPresent,
    accessTokenShapeValid: tokenPresence.vaultConfigured,
