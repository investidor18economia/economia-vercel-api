...
export const MERCADOLIVRE_OAUTH_TOKEN_PERSISTENCE_VERSION = "05J.5";
...
export async function resolveMercadoLivreAccessTokenSource(input = {}) {
  const env = input.env || process.env;
  const allowEnvFallback = input.allowEnvFallback === true;
  ...
  const envToken = normalizeMercadoLivreAccessToken(env.MERCADOLIVRE_ACCESS_TOKEN);
