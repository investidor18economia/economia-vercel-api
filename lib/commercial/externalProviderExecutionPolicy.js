...
import {
  ...
  clearSharedExternalCallAccounting,
  clearSharedRequestExecutionEnv,
  getSharedExternalCallAccounting,
  getSharedRequestExecutionEnv,
  getSharedRequestState,
} from "../miaSharedRequestState.js";

/** @deprecated module fallback — prefer ALS-backed shared request state (PATCH 12F). */
let fallbackActiveExternalCallAccounting = null;
/** @deprecated module fallback — prefer ALS-backed shared request state (PATCH 12F). */
let fallbackActiveRequestExecutionEnv = null;

export function bindActiveRequestExecutionEnv(env = null) {
  const state = getSharedRequestState();
  if (state) {
    return bindSharedRequestExecutionEnv(state, env || null);
  }
  fallbackActiveRequestExecutionEnv = env || null;
  return fallbackActiveRequestExecutionEnv;
}

export function getActiveRequestExecutionEnv() {
  const fromShared = getSharedRequestExecutionEnv();
  if (fromShared) return fromShared;
  return fallbackActiveRequestExecutionEnv || process.env;
}

export function clearActiveRequestExecutionEnv() {
  const state = getSharedRequestState();
  if (state) {
    clearSharedRequestExecutionEnv(state);
    return;
  }
  fallbackActiveRequestExecutionEnv = null;
}

export function bindActiveExternalCallAccounting(enforcementCtx = null) {
  const state = getSharedRequestState();
  if (state) {
    return bindSharedExternalCallAccounting(state, enforcementCtx);
  }
  fallbackActiveExternalCallAccounting =
    enforcementCtx?.externalCallAccounting ||
    enforcementCtx?.providerAccounting?.externalCallAccounting ||
    null;
  return fallbackActiveExternalCallAccounting;
}

export function getActiveExternalCallAccounting() {
  const fromShared = getSharedExternalCallAccounting();
  if (fromShared) return fromShared;
  return fallbackActiveExternalCallAccounting;
}
...
export function clearActiveExternalCallAccounting() {
  const state = getSharedRequestState();
  if (state) {
    clearSharedExternalCallAccounting(state);
    return;
  }
  fallbackActiveExternalCallAccounting = null;
}
