...
export function buildSpecialistDecisionExplanation(input = {}) {
  ...
  if (
    ...
    const tradeoffCommunication = buildTradeoffCommunicationBlock({
      structuredFacts,
      searchCognition,
      decisionMemory,
      query,
  ...
  return finalizeExplanation(paragraphs, {
    ...
    expertInsightApplied: paragraphs.some((entry) =>
      /isso costuma importar|na pr[aá]tica, isso pesa|isso muda a decis[aã]o|n[aã]o conecta|entra na decis[aã]o/i.test(
        entry
      )
    ),
  });
